...
{
	n_GL = 4;						//Two degrees of freedom (this constraint has 4 Lagrange multipliers)
...
bool UniversalJoint::Read(FILE *f)
{
	...
	//TEST for check orthogonality between E1A and E2B
...
void UniversalJoint::Mount()
{
	...
	if (active_lambda[3] == 1)
	{
		...
		QA = I3 + g*(A + 0.5*(A*A));
		ei1A = QA*(*db.CS[csA - 1]->E1);	//Axis e1 in the start of the increment
		
		alpha_escalar_i = norm(alphaiB);
		A = skew(alphaiB);
		g = 4.0 / (4.0 + alpha_escalar_i*alpha_escalar_i);
		QB = I3 + g*(A + 0.5*(A*A));
		ei2B = QB*(*db.CS[csB - 1]->E2);	//Axis e2 in the start of the increment
...
void UniversalJoint::ComputeInitialGuessDisplacements()
{
	...
	if (bool_table.GetAt(db.current_solution_number - 1) == true)
	{
		...
		//Projeção of the velocity angular initial in the system local - for impose only velocity in the GL restritos - impõe same velocity angular in the direction of the axis of the articulação
		...
		Matrix omega_local_B = (*db.CS[csA - 1]->Q)*omega_global_B; //Note: csA and csB possuem same direction E3!
