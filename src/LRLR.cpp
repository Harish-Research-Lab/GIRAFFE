...
LRLR::LRLR()
{
	...
	tol_ortho = 1e-16;	//Tolerance of error the orthogonality
	...
	//Below the variables that dependem of the number of elements for to be allocated - Allocation done in the function Alloc, when called durante the PreCalc
	typeOK1 = NULL;
	typeOK2 = NULL;
	activate = NULL;
	L1 = NULL;
...
void LRLR::WriteMonitor(FILE *f, bool first_record, double time)
{
	...
	//Sum cross of the internal forces in all the nodes of the contact pair
	sum_Fx = 0;
	sum_Fy = 0;
	sum_Fz = 0;
	for (int i = 0; i < n_elements1; i++)
...
void LRLR::SaveLagrange()
{
	for (int i = 0; i < n_elements1; i++)
	{
		for (int j = 0; j < n_elements2; j++)
		{
			//Saves values for chutes initial of the next step
...
//Plots caracteristicas of the contact, independente of ter converged or not
...
void LRLR::Mount()
{
	for (int i = 0; i < n_elements1; i++)
	{
		for (int j = 0; j < n_elements2; j++)
		{
			...
			zeros(c_loading[i][j]);
			//If the contact this active (near) - performs the assembly
			if (activate[i][j] == 1)
			{
				temp_element1 = db.line_regions[n_LR1 - 1]->elements[i];
				temp_element2 = db.line_regions[n_LR2 - 1]->elements[j];
				FillNodes(temp_element1, temp_element2);
				//Changes csi_1, csi_2, N1, N2, dN1, dN2, ddN1 and ddN2 for the point of the possible contact - necessary for construction of functions gap, etc.
				return_value[i][j] = FindMinimumParameters(i, j);
				//Return 0 - localizou point of minimum distance with NR
				//Return 1 - Problems of convergence NR or point this outside of the dominios of the barras with tolerance the orthogonality not obedecida
				*z1 = (*N1)*(*x1);	//z1
				*z2 = (*N2)*(*x2);	//z2
				*z2z1[i][j] = *z2 - *z1;
				//Calculation of the gap normal
				g_n[i][j] = norm(*z2z1[i][j]) - (r1[i] + r2[j]);
				//Case be the first assembly, performs pre-calculation of the distance between the points (for ter reference for check cruzamento of vigas)
				if (first_mount[i][j] == true)
				{
					*last_dif_pos[i][j] = *z2z1[i][j];
					first_mount[i][j] = false;
				}
				//Checks if mudou status in relação the last configuration converged - the result desse algoritmo and the variable boolean flag_cross
				...
				{
					if (dot_test > 0)		//Not there was cruzamento - dot product positive
						flag_cross[i][j] = false;
					else
					{
						if (dot_test == 0)	//There is a point in comum - a point in comum between the vigas - here there is falha if the vigas girarem 90 deg in a step (very raro)
							flag_cross[i][j] = true;
						else				//Muda the status - there was cruzamento of vigas
							flag_cross[i][j] = true;
					}
				}
				
				//If the flag_cross and true - modifies the calculation of the gap normal
				if (flag_cross[i][j] == true)
				{
					g_n[i][j] = -(norm(*z2z1[i][j]) + (r1[i] + r2[j]));
				}

				if (return_value[i][j] == 0)	//Relações of orthogonality obedecidas
				{
					//If there is contact, then performs the contribution
					if (g_n[i][j] <= 0)
					{
						//Calculation of the directions tangenciais of the elements and comprimentos of the elements
						CalculateLengthsAndTangents();
						//CHECK OF Domain OF THE BARRAS
						//double d = mu*abs(g_n[i][j])*epn / ept;
						//d = 0.001;
						////Element 1
						////Point in the interior of the domain
						...
						*ddz1 = (*ddN1)*(*x1);	//ddz1
						*ddz2 = (*ddN2)*(*x2);	//ddz2
						//Construction of matrices auxiliary of the contact
						//A matrix [A] already this construida - and the própria matrix Jacobiana of the algoritmo of search - NR
						//Matrix [B]
						(*B)(0, 0) = (*dz1)(0, 0);
						(*B)(0, 1) = (*dz1)(1, 0);
						(*B)(0, 2) = (*dz1)(2, 0);
						...
						*E2 = +1.0*transp(*dN2)*((*n)*(*d2));
						...
						//Matrix [F] - in the case of elements with interpolation linear this contribution will be nula
						...
						//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//Direction tangential - implementação of the atrito with base in the paper Zavarise and Wriggers (2000)
						//Checks if csi0 must or not be updated (if the contact this começando in this step or not)
						if (last_g_n[i][j] >= 0)
						{
							csi_1_0[i][j] = csi_1[i][j];
							csi_2_0[i][j] = csi_2[i][j];
							//Zeroes possible acumulo of displacement previous - the contact this reiniciando - not there is deslizamento acumulado
							gt1s[i][j] = 0.0;
							gt2s[i][j] = 0.0;
						}
						else
						{
							//If the point caiu outside of the domain of through the less a of the barras in the last step converged (can be indicativo of deslizamento of element vizinho)
							if (last_return_value[i][j] == 3)
							{
								...
								////Calculation of the magnitude of the deslizamento between the point of outside of the domain and the point current (in the domain)
								//double factor = 
								;
								...
								////If this delizando consideravelmente - calculates csi_1_0 and csi_2_0 ficticios
						...
						*gte = gte1*(*t1) + gte2*(*t2);	//elastico
						...
						//Magnitude of the force of atrito that of fato ocorrera, after the devidas verificações the to be processadas the seguir:
						//Check of sticking or slipping
						////////////////////////////Sliding//////////////////////////////
						if (Fat_try >= Fat_max)
						{
							sticking[i][j] = false;
							Fat = Fat_max;
							//Calculates the deslizamento
							if (ept != 0.0)
								delta_lambda = (Fat_try - Fat_max) / ept;	//Magnitude of the update of deslizamento
							else
								delta_lambda = 0.0;
							//Updates the gap of deslizamento (but só of fato will be computed in the case of convergence of the NR)
							if (norm(*gte) != 0.0)
							{
								if (delta_lambda != 0.0)
								{
						...
						//In this moment already if sabe if stick or slip. Agora calculation of the contributions for way fraca and operator tangent
						for (int ii = 0; ii<3; ii++)
						{
							(*t1ext)(ii, 0) = (*t1)(ii, 0);
							(*t2ext)(ii + 3, 0) = (*t2)(ii, 0);
						}
						//Matrix auxiliary for the calculation of G3 - identidade of order 3 with zeros in the remaining - I361 and I362
						for (int ii = 0; ii<3; ii++)
						{
							(*I361)(ii, ii) = 1.0;
							(*I362)(ii + 3, ii + 3) = 1.0;
						...
						*G7b = ((dot12 / (D_*D_))*(*G4) - ((1.0*dot11) / (D_*D_))*(*G4b)) * (*G5) -
						...
						//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//Method of penalidades - construction of the internal forces of contact and of the stiffness matrix tangent
					...
					else//If not there is contact
					{
						//Deslizamentos acumulados null
						g_t1s_temp[i][j] = 0.0;
						g_t2s_temp[i][j] = 0.0;
					}
				}//return_value == 0
...
{
	//Checks the values maximos of the GLs of the elements 1 and 2
	temp_band_free = 0;
	temp_band_fixed = 0;
	...
	for (int ei = 0; ei < n_elements1; ei++)
	{
		for (int ej = 0; ej < n_elements2; ej++)
		{
			if (return_value[ei][ej] == 0 && g_n[ei][ej] < 0)
			{
				...
				for (int j = 0; j < 3; j++)//Iterates over the 3 primeiros degrees of freedom of each node (this contact só afeta these degrees of freedom, not envolve the rotations)
				{
					//Checando each a of the nodes of the element 1 
					...
					//Checando each a of the nodes of the element 2
					for (int node = 0; node < 3; node++)
					{
...
void LRLR::PreCalc()
{
	...
	n_elements1 = db.line_regions[n_LR1 - 1]->n_elements;		//Number of elements in the line region 1
	n_elements2 = db.line_regions[n_LR2 - 1]->n_elements;		//Number of elements in the line region 2
	Alloc(n_elements1, n_elements2);							//Allocation dynamics, according to the number of elements of the LR1 and LR2
	...
	for (int i = 0; i < n_elements1; i++)
	{
		...
		{
			typeOK1[i] = 1;
			Pipe_1* ptr_element = static_cast<Pipe_1*>(db.elements[temp_element1 - 1]);	//ptr_element is the pointer to element Pipe_1, index i from line_region
		...
		else
		{
			...
			{
				typeOK1[i] = 1;
				Beam_1* ptr_element = static_cast<Beam_1*>(db.elements[temp_element1 - 1]);	//ptr_element is the pointer to element Beam_1, index i from line_region
	...
	for (int i = 0; i < n_elements2; i++)
	{
		...
		{
			typeOK2[i] = 1;
			Pipe_1* ptr_element = static_cast<Pipe_1*>(db.elements[temp_element2 - 1]);	//ptr_element is the pointer to element Pipe_1, index i from line_region
			//Calculations initial
		...
		else
		{
			...
			{
				typeOK2[i] = 1;
				Beam_1* ptr_element = static_cast<Beam_1*>(db.elements[temp_element2 - 1]);	//ptr_element is the pointer to element Beam_1, index i from line_region
...
//Allocates in the memory the variables that dependem of the number of elements
...
//Fills the variables of the nodes with values atualizados
...
//Algoritmo for determination of csi_1 and csi_2 - algoritmo of minimização of distance
//return 0 - returns values of csi_1 and csi_2
int LRLR::FindMinimumParameters(int i, int j)
{
	//Stores cópias of variables convergidas in the last iteration - will be usadas case the NR not convirja (by exemplo, if there is paralelismo)
	double copy_csi_1 = csi_1[i][j];
	double copy_csi_2 = csi_2[i][j];
	int flag_error = 0;
	//Initialization of initial guess - the last value converged in the NR
	csi_1[i][j] = copy_csi_1_converged[i][j];
	csi_2[i][j] = copy_csi_2_converged[i][j];
	error = tol_ortho + 1.0;	//Forcing enter in the loop
	int it = 1;
	if (error > tol_ortho)
	{
		//LOOP main
		while (error > tol_ortho && it <= max_it)
		{
			...
			error = norm(*R);	//Norm infinito
			//Só does more a iteration, if still not converged - case contrario all the variables already ficam saved with the last value converged
			//This and importante since all these variables are used for the assembly of the stiffness matrix of the contact
			if (error > tol_ortho)
			{
	...
	//Point outside of the domain of the bar
	if (abs(csi_1[i][j]) > 1.0 || abs(csi_2[i][j]) > 1.0)
	{
		//If for a point of the extremidades (tips) of the line region
	...
	//Case chegue here significa that retornara the point of the Newton-Raphson, converged devidamente and that obedece the relações of orthogonality
	return 0;
}

//Calculates parameters as a function of csi_1 and csi_2
...
//Calculates the vectors t1 and t2 of the elements
...
//Returns 0 - not there is cruzamento 1 - there is cruzamento
bool LRLR::HaveErrors()
{
	for (int i = 0; i < n_elements1; i++)
	{
		...
		for (int j = 0; j < n_elements2; j++)
		{
			if (activate[i][j] == 1 && g_n[i][j] < 0.0 && flag_cross[i][j] == true && return_value[i][j] == 0)	//Check of "cross"
			{
				db.myprintf("\nLRLR Contact %d\n", number);
				temp_element1 = db.line_regions[n_LR1 - 1]->elements[i];
				temp_element2 = db.line_regions[n_LR2 - 1]->elements[j];
