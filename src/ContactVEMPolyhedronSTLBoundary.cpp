...
ContactVEMPolyhedronSTLBoundary::ContactVEMPolyhedronSTLBoundary()
{
	index1 = 0;				//Particle 1 - index
	index2 = 0;				//Boundary 2 - index
	sub_index1 = 0;			//Particle 1 - sub_index
	sub_index2 = 0;			//Boundary 2 - sub_index
...
void ContactVEMPolyhedronSTLBoundary::PreCalc()
{
	pA = static_cast<VEMPolyhedron*>(db.particles[index1]);
	pB = static_cast<STLBoundary*>(db.boundaries[index2]);
...
void ContactVEMPolyhedronSTLBoundary::InsertNewContact(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
	FlexibleTriangularFace_RigidTriangularFace* temp = new FlexibleTriangularFace_RigidTriangularFace();
	temp->index1 = index1;
	temp->index2 = index2;
...
int ContactVEMPolyhedronSTLBoundary::CreateSurfacePair(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
	int list_index = -1;
	for (int i = 0; i < contact_pairs.size(); i++)
	{
		FlexibleTriangularFace_RigidTriangularFace* ptr = static_cast<FlexibleTriangularFace_RigidTriangularFace*>(contact_pairs[i]);
...
void ContactVEMPolyhedronSTLBoundary::ProcessSurfacePair(int list_index)
{
	contact_pairs[list_index]->eligible = false;
	contact_pairs[list_index]->SolveLCP();
	contact_pairs[list_index]->EvaluateNormalGap();
}
void ContactVEMPolyhedronSTLBoundary::ProcessSurfacePairs()
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
		ProcessSurfacePair(i);
}

void ContactVEMPolyhedronSTLBoundary::FinalProcessSurfacePairExplicit(int list_index, double t)
{
	//Copies
	...
	contact_pairs[list_index]->SolveLCP();
	contact_pairs[list_index]->EvaluateNormalGap();//here the normal of contact is updated, which ensures the successful test for penetration in HaveErrors function
...
void ContactVEMPolyhedronSTLBoundary::FinalProcessSurfacePairsExplicit(double t)
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
		FinalProcessSurfacePairExplicit(i,t);
}

void ContactVEMPolyhedronSTLBoundary::MountGlobalExplicit()
{
	//ALL-Explicit
}
...
void ContactVEMPolyhedronSTLBoundary::MountGlobal()
{
	...
	{
		FlexibleTriangularFace_RigidTriangularFace* ptr = static_cast<FlexibleTriangularFace_RigidTriangularFace*>(contact_pairs[c]);
		if (contact_pairs[c]->GetActive())
		{
			if (contact_pairs[c]->eligible)
			{
				for (int i = 0; i < 15; i++)
				{
					...
					if (i >= 9 && i < 12)
						GL_global_1 = db.nodes[db.boundaries[index2]->node - 1]->GLs[i - 9];
					if (i >= 12 && i < 15)
						GL_global_1 = db.nodes[db.boundaries[index2]->node - 1]->GLs[i - 12];
					...
					if (GL_global_1 > 0)
					{
						...
						//previous = db.global_ABS_P_A(GL_global_1 - 1, 0);
						//db.global_ABS_P_A(GL_global_1 - 1, 0) = previous + abs(ptr->Rc[i]);
					...
					if (GL_global_1 < 0)
					{
						...
						////Convergence criteria
						//previous = db.global_ABS_P_B(-GL_global_1 - 1, 0);
						//db.global_ABS_P_B(-GL_global_1 - 1, 0) = previous + abs(ptr->Rc[i]);
					...
					for (int j = 0; j < 15; j++)
					{
						...
						if (j >= 9 && j < 12)
							GL_global_2 = db.nodes[db.boundaries[index2]->node - 1]->GLs[j - 9];
						if (j >= 12 && j < 15)
							GL_global_2 = db.nodes[db.boundaries[index2]->node - 1]->GLs[j - 12];
...
void ContactVEMPolyhedronSTLBoundary::ProcessContactHierarchy()
{
	for (int c = 0; c < (int)contact_pairs.size(); c++)
	{
		...
		if (cur_active && contact_pairs[c]->eligible)
		{
			...
			STLBoundary* pB = static_cast<STLBoundary*>(db.boundaries[index2]);
			STLSurface* surfA;
			STLSurface* surfB;
			...
			if (deg_curveA != 0 && deg_pointB != 0)
			{
				...
				if (surfA->edges[deg_curveA - 1].concave_indicator <= 0)
				{
					...
					bool checked = false;
					for (int subcont = 0; subcont < db.gcs->contactPB_list[index1].size() && checked == false; subcont++)
					{
						//Check if are the same particles, but not the same pair 
						if (db.gcs->contactPB_list[index1][subcont]->index2 == index2 &&
							(db.gcs->contactPB_list[index1][subcont]->sub_index1 != sub_index1 ||
								db.gcs->contactPB_list[index1][subcont]->sub_index2 != sub_index2))
						{
							for (int subc = 0; subc < (int)db.gcs->contactPB_list[index1][subcont]->contact_pairs.size() && checked == false; subc++)
							{
								//If the pair is active and eligible
								if (db.gcs->contactPB_list[index1][subcont]->cur_active && db.gcs->contactPB_list[index1][subcont]->contact_pairs[subc]->eligible)
								{
									ContactVEMPolyhedronSTLBoundary* subptr = static_cast<ContactVEMPolyhedronSTLBoundary*>(db.gcs->contactPB_list[index1][subcont]);
