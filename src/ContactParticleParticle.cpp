...
ContactParticleParticle::ContactParticleParticle()
{
}
...
void ContactParticleParticle::Clear()
{
	for (int i = 0; i < contact_pairs.size(); i++)
		contact_pairs[i]->SetUnnactive();
}

bool ContactParticleParticle::HaveErrors()
{
...
void ContactParticleParticle::SaveConfiguration()
{
...
void ContactParticleParticle::MountContactsExplicit(double t)
{
	for (int i = 0; i < contact_pairs.size(); i++)
	{
		if (contact_pairs[i]->eligible)
...
bool ContactParticleParticle::NightOwlContact()
{
	ProcessSurfacePairs();
	...
	for (int i = 0; i < contact_pairs.size(); i++)
	{
		...
		if (contact_pairs[i]->eligible == true)
		{
			...
			if (typeid(*db.particles[index2]->sub_bv[sub_index2]) == typeid(BoundingSphere))
				db.myprintf("BoundingSphere\n");
			if (typeid(*db.particles[index2]->sub_bv[sub_index2]) == typeid(BoundingCylinder))
			{
				db.myprintf("BoundingCylinder\n");
...
{
	double return_step = db.solution[db.current_solution_number - 1]->end_time;				//value alto
	for (int i = 0; i < contact_pairs.size(); i++)
	{
