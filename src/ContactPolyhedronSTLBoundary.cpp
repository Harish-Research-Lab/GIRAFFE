...
ContactPolyhedronSTLBoundary::ContactPolyhedronSTLBoundary()
{
...
void ContactPolyhedronSTLBoundary::PreCalc()
{
...
void ContactPolyhedronSTLBoundary::InsertNewContact(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
...
int ContactPolyhedronSTLBoundary::CreateSurfacePair(int deg_pointA, int deg_curveA, int deg_pointB, int deg_curveB, int faceAID, int faceBID)
{
	int list_index = -1;
	for (int i = 0; i < contact_pairs.size(); i++)
	{
...
void ContactPolyhedronSTLBoundary::ProcessSurfacePair(int list_index)
{
	contact_pairs[list_index]->eligible = false;
	contact_pairs[list_index]->SolveLCP();
	contact_pairs[list_index]->EvaluateNormalGap();
}
void ContactPolyhedronSTLBoundary::ProcessSurfacePairs()
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
		ProcessSurfacePair(i);
}

void ContactPolyhedronSTLBoundary::FinalProcessSurfacePairExplicit(int list_index, double t)
{
	contact_pairs[list_index]->eligible = false;
	contact_pairs[list_index]->SolveLCP();
...
void ContactPolyhedronSTLBoundary::FinalProcessSurfacePairsExplicit(double t)
{
	for (int i = 0; i < (int)contact_pairs.size(); i++)
...
void ContactPolyhedronSTLBoundary::MountGlobalExplicit()
{
	...
	//Note: here the contacts are subtraidos since in the explicito already this isolada the acceleration and here entram as if fosse forces external, being necessary the inversão of the sign
	int GL_global_1 = 0;
	double anterior = 0;
	for (int c = 0; c < contact_pairs.size(); c++)
	{
		if (contact_pairs[c]->GetActive())
		{
			...
			{
				//db.PrintPtr(contact_pairs[c]->Rc, 12);
				//contact_pairs[c]->cd->Plot();
				for (int i = 0; i < 12; i++)
				{
					if (i < 6)
						GL_global_1 = db.nodes[db.particles[index1]->node - 1]->GLs[i];
					else
						GL_global_1 = db.nodes[db.boundaries[index2]->node - 1]->GLs[i - 6];
				...
				for (int i = 0; i < 12; i++)
				{
					...
					for (int j = 0; j < 12; j++)
					{
						if (j < 6)
							...
							GL_global_2 = db.nodes[db.boundaries[index2]->node - 1]->GLs[j - 6];
...
void ContactPolyhedronSTLBoundary::ProcessContactHierarchy()
{
					...
					for (int subcont = 0; subcont < db.gcs->contactPB_list[index1].size() && checked == false; subcont++)
					{
								...
								db.gcs->contactPB_list[index1][subcont]->sub_index2 != sub_index2))
						{
							for (int subc = 0; subc < (int)db.gcs->contactPB_list[index1][subcont]->contact_pairs.size() && checked == false; subc++)
							{
								...
								{
									ContactPolyhedronSTLBoundary* subptr = static_cast<ContactPolyhedronSTLBoundary*>(db.gcs->contactPB_list[index1][subcont]);
