...
ContactBodyBody::ContactBodyBody()
{
}
...
void ContactBodyBody::Clear()
{
	
}

void ContactBodyBody::DefaultValues()
{
...
void ContactBodyBody::SaveConfiguration()
{
...
void ContactBodyBody::MountContact()
{
	if (eligible)
	{
		Alloc();
...
void ContactBodyBody::MountContactExplicit(double t)
{
	//ALL-Explicit
}

bool ContactBodyBody::NightOwlContact()
{
...
void ContactBodyBody::PreCalc()
{
	//Pointers to surfaces
	if (invert)
	{
		sA = static_cast<Geometry*>(db.body_geometries[index2]->ptr_geom[sub_index2]);
		sB = static_cast<Geometry*>(db.body_geometries[index1]->ptr_geom[sub_index1]);
	}
	else
	{
		sA = static_cast<Geometry*>(db.body_geometries[index1]->ptr_geom[sub_index1]);
	...
	if (sA->deg_u1 == true || sA->deg_u2 == true || sB->deg_u1 == true || sB->deg_u2 == true)
	...
	cd->deg_control[0][0] = sA->deg_u1;
	cd->deg_control[0][1] = sA->deg_u2;
	...
	cd->copy_deg_coordinates[0][0] = sA->deg_u1_value;
	cd->copy_deg_coordinates[0][1] = sA->deg_u2_value;
	...
	{
		if ((db.contactinterfaces[i]->material_1 == sA->material && db.contactinterfaces[i]->material_2 == sB->material) ||
			(db.contactinterfaces[i]->material_1 == sB->material && db.contactinterfaces[i]->material_2 == sA->material))
		{
...
void ContactBodyBody::EvaluateNormalGap()
{
...
void ContactBodyBody::SolveLCP()
{
	...
	//Search of point of minimum or saddle - according to the type of problem and degeneration
	if (interface_0_flag)
	{
		//No degeneration
		if (cd->degenerated[0] == false)
		{
			//Direct search for saddle
			if ((cd->return_value[0] == 0 || cd->return_value[0] == 4) /*&& eligible*/)
			{
				if (write_report)
					fprintf(f_TR_report, "Direct search for saddle\n");
				converged2 = FindSaddleSolution(cd, &cb, info, false);
			}
			...
			if (converged2 == false)
			{
				...
				if (CharacterizeCriticalPoint(&cb) == 1)
					converged2 = FindSaddleSolution(cd, &cb, info, false);
		...
		else
		{
			...
			{
				if (write_report)
					fprintf(f_TR_report, "Direct search for saddle or minimum\n");
				if (cd->P_0[0]->getColumns() == 3)
					converged2 = FindSaddleSolutionDegenerated(cd, cd->P_0[0], &cb, false);
				else
					converged2 = FindMinimumSolutionDegenerated(cd, cd->P_0[0], &cb);
			}
			//If not entrou in the if previous or if not converged the if previous
			if (converged2 == false)
			{
				...
				if (cd->P_0[0]->getColumns() == 3)
				{
					...
					if (CharacterizeCriticalPointDegenerated(&cb, cd->P_0[0]) == 1)
						converged2 = FindSaddleSolutionDegenerated(cd, cd->P_0[0], &cb, false);
					else
						converged2 = converged1;
				}
	...
	if (previous_evaluation == false)
	{
		...
		else
		{
			if (cd->return_value[0] == 0)
				cd->return_value[0] = 1;
			//db.myprintf("\nUnconverged LCP between surfaces %d and %d.\n", sA->number, sB->number);
		}
...
bool ContactBodyBody::HaveErrors()
{
	...
	{
		db.myprintf("\nUnconverged LCP between Geometry %d and Geometry %d. Check results carefully!\n", sA->number, sB->number);
		//return true;
	}

	if (interface_0_flag == false)
	{
		/*if (cd->copy_return_value[0] == 0 && cd->copy_characterization_index[0] == 0 && cd->characterization_index[0] == 1)
		{
			db.myprintf("Error in contact between Geometry %d and Geometry %d\n", sA->number, sB->number);
			cd->Plot();
			return true;
		}*/
		...
		{
			if (cd->return_value[0] == 0 && cd->copy_return_value[0] == 0)
			{
				db.myprintf("Error in contact between Geometry %d and Geometry %d\n", sA->number, sB->number);
				cd->Plot();
				return true;
...
void ContactBodyBody::SolvePreviousContact()
{
	...
	*cd->copy_g[0] = *GammaA - *GammaB;
	...
	if (interface_0_flag)
	{
		...
		Matrix nA(3);
		Matrix nB(3);
		sA->NormalExt(cd->copy_convective[0][0], cd->copy_convective[0][1], &nA, true);
		sB->NormalExt(cd->copy_convective[0][2], cd->copy_convective[0][3], &nB, true);
		Matrix xk(4);
		xk(0, 0) = cd->copy_convective[0][0];
		xk(1, 0) = cd->copy_convective[0][1];
		xk(2, 0) = cd->copy_convective[0][2];
		xk(3, 0) = cd->copy_convective[0][3];
		if (dot(nA, nB) < 0)
			passed_normal_test = true;
		if (Gap(xk, false, nA, nB) <= 0.0  && passed_normal_test)
			cd->copy_g_n[0] = -1.0*norm(*cd->copy_g[0]);
		else
			cd->copy_g_n[0] = +1.0*norm(*cd->copy_g[0]);
	}
	else
...
void ContactBodyBody::SurfacePoints()
{
	double z;
	double th;
	//Gamma A
	if (previous_evaluation == false)
	{
		...
		sA->SurfacePosition(GammaA, z, th, true);
	}
	else
	{
		z = cd->copy_convective[0][0];
		th = cd->copy_convective[0][1];
		sA->SurfacePosition(GammaA, z, th, false);
	}
...
void ContactBodyBody::ProcessSurfacePair()
{
	eligible = false;
	SolveLCP();
	EvaluateNormalGap();
...
void ContactBodyBody::FinalProcessSurfacePairsExplicit(double t)
{
	eligible = false;
	SolveLCP();
	EvaluateNormalGap();
...
void ContactBodyBody::WriteVTK_XMLForces(FILE *f)
{
...
void ContactBodyBody::EvaluateInvertedHessian()
{
	Matrix invHes(4, 4);
...
void ContactBodyBody::MountGlobal()
{
	//Pointers to surfaces
	...
	if (eligible)
	{
		//cd->Plot();
		//db.PrintPtr(Rc, 24);
		//db.PrintPtr(Kc, 24,24);
		for (int i = 0; i < (sA->nDOFs + sB->nDOFs); i++)
		{

			if (i < sA->nDOFs)
				GL_global_1 = *sA->GLs[i];															//GLs of the surface 1
			else
				GL_global_1 = *sB->GLs[i - sA->nDOFs];											//GLs of the surface 2
			...
			{
				if (GL_global_1 != 0)//if the GL and active
				{
			...
			for (int j = 0; j < (sA->nDOFs + sB->nDOFs); j++)
			{
				if (j < sA->nDOFs)
					GL_global_2 = *sA->GLs[j];									//GLs of the surface 1
				else
					GL_global_2 = *sB->GLs[j - sA->nDOFs];					//GLs of the surface 2
...
void ContactBodyBody::MountGlobalExplicit()
{
	//ALL-Explicit
}

//Optimization - determination of minimum
bool ContactBodyBody::FindMinimumSolution(SSContactData* c_data, Matrix* solution, int &return_info)
{
	//Data - trust region
...
bool ContactBodyBody::FindSaddleSolution(SSContactData* c_data, Matrix* solution, int &return_info, bool return_gap)
{
	//Data - trust region
...
//Optimization - determination of minimum
bool ContactBodyBody::FindMinimumSolutionDegenerated(SSContactData* c_data, Matrix* P_0, Matrix* solution)
{
	//Data - trust region
	double Deltamax = 1e4;			//maximum radius of the trust region allowed
	double Deltak = 0.1;			//current radius of trust region
...
//Optimization - determination of minimum
bool ContactBodyBody::FindMinimumGapDegenerated(SSContactData* c_data, Matrix* P_0, Matrix* solution, int &return_info, bool fixed_normals, Matrix& nA, Matrix& nB)
{
	//Data - trust region   
...
//Optimization - determination of saddle
bool ContactBodyBody::FindSaddleSolutionDegenerated(SSContactData* c_data, Matrix* P_0, Matrix* solution, bool return_gap)
{
...
int ContactBodyBody::CharacterizeCriticalPoint(Matrix* solution)
{
...
int ContactBodyBody::CharacterizeCriticalPointDegenerated(Matrix* solution, Matrix* P_0, bool print)
{
	///////////////////////////////Pointers and variables to make access easier//////////////////////////////////////////
	//Surface* surf1 = db.surfaces[surf1_ID - 1];		//Pointer to the surface 1
	//Surface* surf2 = db.surfaces[surf2_ID - 1];		//Pointer to the surface 2
...
void ContactBodyBody::OpenTRReport()
{
	//Writing the name of the contact pair
	char pair_name[100];
	sprintf(pair_name, "TR_report_surf_%d_surf_%d", db.body_geometries[index1]->ptr_geom[sub_index1]->number, db.body_geometries[index2]->ptr_geom[sub_index2]->number);
...
void ContactBodyBody::InitializeTRReport()
{
	if (write_report)
	{
		//Writing the name of the contact pair
