...
Table::Table(int e_times, int e_values)
{
	...
	//Allocation of the vector of values
	...
	//Zeroes all the components of the vectors recem-allocated
	for (int i = 0; i < n_times; i++)
	{
		times[i] = 0;
...
//Returns the value in the instant required (interpolation linear)
double Table::GetValueAt(double time_value, int value_index)
{
	//Performs the interpolation linear
	double return_value = 0;

	if (n_times == 0)
		return return_value;
	//If for larger that the last time, returns the last
	if (time_value >= times[n_times - 1])
		return values[n_times - 1][value_index];
	//If for smaller that the first time, returns the first
...
bool  Table::Read(FILE *f)
{
	...
	{
		fscanf(f, "%s", s);
		if (!strcmp(s, "Time"))//keyword optional ("Time")
			fscanf(f, "%s", s);
		times[i] = atof(s);
		for (int j = 0; j < n_values; j++)
		{
