...
bool NURBSParticle::Read(FILE *f)
{
	char s[1000];
	fscanf(f, "%s", s);
	number = atoi(s);
...
void NURBSParticle::Write(FILE *f)
{
	fprintf(f, "NURBS\t%d\tMat\t%d\tCS\t%d\tCADData\t%d\tNode\t%d\n",
		number,
		material,
		cs,
		CADDATA_ID,
...
void NURBSParticle::WriteModifyingParameters(FILE *f, int e_material, int e_node, int e_number, int e_cs)
{
	fprintf(f, "NURBS\t%d\tMat\t%d\tCS\t%d\tCADData\t%d\tNode\t%d\n",
		e_number,
		e_material,
...
void NURBSParticle::WriteVTK_XMLRender(FILE *f)
{
	Matrix pos(3);
...
void NURBSParticle::MountFieldLoading()
{
